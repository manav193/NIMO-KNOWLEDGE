...
### Runtime Consumption Contract & Approval Boundary
NIMO-CORE acts as the intelligence and orchestration layer, while NIMO-KNOWLEDGE acts as the authoritative governed knowledge layer:
1. **Approval Enforcement**: NIMO-CORE strictly validates `indexes/catalog.json` and only consumes entries with `status === "approved"` or `status === "active"`.
2. **Rejection of Unapproved Content**: Entries with status `draft`, `proposed`, `evaluated`, `rejected`, or `archived` are ignored and never loaded into runtime memory.
3. **No Autonomous Self-Modification**: Learning observations and improvement proposals generated by NIMO-CORE remain strictly in proposal state until reviewed and approved via the NIMO-KNOWLEDGE governance gate.
4. **Google Drive Division**: Google Drive serves as a planned future bulk-data/storage layer for raw multi-gigabyte logs, audio, and benchmarks. No live Google Drive integration exists in this phase.
